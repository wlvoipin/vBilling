...
class Customer_model extends CI_Model {
	...
	*/
	...
	//customer rates 
	function customer_rates($num, $offset, $tbl_name, $filter_start_date, $filter_end_date, $filter_rate_type, $filter_sort)
	{
		if($offset == ''){$offset='0';}
	...
	function customer_rates_count($customer_group_table, $filter_start_date, $filter_end_date, $filter_rate_type)
	{
		$where = '';
		$where .= "WHERE id != '' ";
...
$query = $this->db->query($sql); 
    ...
    //***************************** CDR FUNCTIONS ***************************************//
    function customer_cdr($num, $offset, $customer_id, $filter_date_from, $filter_date_to, $filter_phonenum, $filter_call_type, $duration_from, $duration_to, $filter_sort)
    {
        if($offset == ''){$offset='0';}
    ...
    function customer_cdr_count($customer_id, $filter_date_from, $filter_date_to, $filter_phonenum, $filter_call_type, $duration_from, $duration_to)
    {
        $where = '';
    ...
    function my_balance($customer_id)
    {
    ...
    function get_invoices_count($filter_date_from, $filter_date_to, $filter_customers, $filter_billing_type, $filter_status)
    {
        $where = '';
        $where .= "WHERE id != '' ";
    ...
    /************for reseller**************/
    function insert_new_acl_node_reseller($customer_id, $ip, $cidr)
	{
		$added_by = $this->session->userdata('customer_id');
    ...
    function insert_new_sip_access_reseller($customer_id, $username, $password, $domain, $sofia_id, $cid)
	{
		$added_by = $this->session->userdata('customer_id');
		$new_password = $username.':'.$domain.':'.$password;
		$new_password = md5($new_password);
