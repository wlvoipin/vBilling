...
class Cdr_model extends CI_Model {
	...
	//get all cdr data 
	function get_all_cdr_data($num, $offset, $filter_date_from, $filter_date_to, $filter_phonenum, $filter_customers, $filter_groups, $filter_call_type, $duration_from, $duration_to, $filter_sort, $filter_contents)
	{
		if($offset == ''){$offset='0';}
        ...
        {
            $where .= "WHERE parent_id = '0' && (parent_reseller_id = '".$this->session->userdata('customer_id')."' || grand_parent_reseller_id = '".$this->session->userdata('customer_id')."') ";
        }
        else if($filter_contents == 'my') //only get those which are created by him 
        {
            $where .= "WHERE parent_id = '0' && parent_reseller_id = '".$this->session->userdata('customer_id')."' ";
        }
        else //show all 
        {
            $where .= "WHERE (parent_id = '".$this->session->userdata('customer_id')."' OR grand_parent_id = '".$this->session->userdata('customer_id')."')";
        }
	...
	function get_cdr_main_count($filter_date_from, $filter_date_to, $filter_phonenum, $filter_customers, $filter_groups, $filter_call_type, $duration_from, $duration_to, $filter_contents)
	{
		$where = '';
    ...
    /*********************MY CDR DATA FUNCTIONS ************************************/
    //get all cdr data 
	function get_all_my_cdr_data($num, $offset, $filter_date_from, $filter_date_to, $filter_phonenum, $filter_call_type, $duration_from, $duration_to, $filter_sort)
	{
		if($offset == ''){$offset='0';}
		...
		$where = '';
		$where .= "WHERE parent_id = '0' && customer_id = '".$this->session->userdata('customer_id')."' ";
	...
	//cdr count for pagination 
	function get_my_cdr_main_count($filter_date_from, $filter_date_to, $filter_phonenum, $filter_call_type, $duration_from, $duration_to)
	{
		$where = '';
	...
	function get_gateway_total_answered_calls($gateway, $sofia_id, $date_from, $date_to)
	{
		...
		$sql = "SELECT COUNT(*) AS total_answered_calls FROM cdr WHERE gateway = '".$gateway."' && sofia_id = '".$sofia_id."' && (hangup_cause = 'NORMAL_CLEARING' || hangup_cause = 'ALLOTTED_TIMEOUT') && (created_time >= '".sprintf("%.0f", $date_from)."' && created_time <= '".sprintf("%.0f", $date_to)."') ";
		$query = $this->db->query($sql);
		$row = $query->row();
	...
	function get_gateway_total_busy_calls($gateway, $sofia_id, $date_from, $date_to)
	{
		...
		$sql = "SELECT COUNT(*) AS total_busy_calls FROM cdr WHERE gateway = '".$gateway."' && sofia_id = '".$sofia_id."' && hangup_cause = 'USER_BUSY' && (created_time >= '".sprintf("%.0f", $date_from)."' && created_time <= '".sprintf("%.0f", $date_to)."') ";
		$query = $this->db->query($sql);
		$row = $query->row();
	...
	function get_gateway_total_rejected_calls($gateway, $sofia_id, $date_from, $date_to)
	{
		...
		$sql = "SELECT COUNT(*) AS total_rejected_calls FROM cdr WHERE gateway = '".$gateway."' && sofia_id = '".$sofia_id."' && hangup_cause = 'CALL_REJECTED' && (created_time >= '".sprintf("%.0f", $date_from)."' && created_time <= '".sprintf("%.0f", $date_to)."') ";
		$query = $this->db->query($sql);
		$row = $query->row();
	...
	function get_gateway_total_failed_calls($gateway, $sofia_id, $date_from, $date_to)
	{
		...
		$sql = "SELECT COUNT(*) AS total_failed_calls FROM cdr WHERE gateway = '".$gateway."' && sofia_id = '".$sofia_id."' && hangup_cause != 'NORMAL_CLEARING' && hangup_cause != 'ALLOTTED_TIMEOUT' && (created_time >= '".sprintf("%.0f", $date_from)."' && created_time <= '".sprintf("%.0f", $date_to)."') ";
		$query = $this->db->query($sql);
		$row = $query->row();
	...
	function get_customer_total_mins($customer_id, $time_period)
	{
		...
		$sql = "SELECT SUM(billsec) AS total_seconds FROM cdr WHERE customer_id = '".$customer_id."' AND (hangup_cause = 'ALLOTTED_TIMEOUT' || hangup_cause = 'NORMAL_CLEARING') AND billsec > 0 AND parent_id = '0' ".$where." ";
		$query = $this->db->query($sql);
		$row = $query->row();
		return $row->total_seconds;
	}
	...
	function get_customer_total_calls($customer_id, $time_period)
	{
		...
		$sql = "SELECT COUNT(*) AS total_calls FROM cdr WHERE customer_id = '".$customer_id."' AND (hangup_cause = 'ALLOTTED_TIMEOUT' || hangup_cause = 'NORMAL_CLEARING') AND billsec > 0 AND parent_id = '0' ".$where."";
		$query = $this->db->query($sql);
		$row = $query->row();
		return $row->total_calls;
	}
	...
	function get_customer_total_sell_cost($customer_id, $time_period)
	{
        ...
        if(customer_any_cell($this->session->userdata('customer_id'), 'reseller_level') == '3'){
            ...
            else if(customer_any_cell($customer_id, 'parent_id') != $this->session->userdata('customer_id') && customer_any_cell($customer_id, 'grand_parent_id') == $this->session->userdata('customer_id'))
            {
                ...
                $sql = "SELECT SUM(total_reseller_sell_cost) AS total_sell_cost FROM cdr WHERE customer_id = '".$customer_id."' AND (hangup_cause = 'NORMAL_CLEARING' || hangup_cause = 'ALLOTTED_TIMEOUT') AND billsec > 0 ".$where."";
            }
        } else {
            $sql = "SELECT SUM(total_sell_cost) AS total_sell_cost FROM cdr WHERE customer_id = '".$customer_id."' AND (hangup_cause = 'NORMAL_CLEARING' || hangup_cause = 'ALLOTTED_TIMEOUT') AND billsec > 0 ".$where."";
        }
	...
	//get customer total mins 
	function get_country_total_mins($country_id, $filter_date_from, $filter_date_to, $filter_contents)
	{
		$where = '';
		...
		$sql = "SELECT SUM(billsec) AS total_seconds FROM cdr ".$where." ";
		$query = $this->db->query($sql);
		$row = $query->row();
		return $row->total_seconds;
	}
	...
	//get customer total calls 
	function get_country_total_calls($country_id, $filter_date_from, $filter_date_to, $filter_contents)
	{
		$where = '';
        if($filter_contents == 'all')
        {
		...
		$sql = "SELECT COUNT(*) AS total_calls FROM cdr ".$where."";
		$query = $this->db->query($sql);
		$row = $query->row();
		return $row->total_calls;
	}
	...
	//get customer total sell cost
	function get_country_total_sell_cost($country_id, $filter_date_from, $filter_date_to, $filter_contents)
	{
		$where = '';
        if($filter_contents == 'all')
        {
        ...
        if(customer_any_cell($this->session->userdata('customer_id'), 'reseller_level') == '3'){
            ...
            foreach($query->result() as $row)
            {
                ...
                {
                    $total_sell_cost = $total_sell_cost + $row->total_sell_cost;
                }
                ...
                {
                    $total_sell_cost = $total_sell_cost + $row->total_reseller_sell_cost;
                }
            }
        } else {
            //user is reseller 2
            $sql = "SELECT SUM(total_sell_cost) AS total_sell_cost FROM cdr ".$where."";
            $query = $this->db->query($sql);
            $row = $query->row();
            $total_sell_cost = $row->total_sell_cost;
        }
        return $total_sell_cost;
	}
	...
	/*********************** EXPORT FUNCTIONS ***************************/
	function export_cdr_data($filter_date_from, $filter_date_to, $filter_phonenum, $filter_customers, $filter_groups, $filter_call_type, $duration_from, $duration_to, $filter_sort, $filter_contents)
	{
		$order_by = "";
        if($filter_sort == 'date_asc')
        {
        ...
        $where = '';
		if($filter_contents == 'all') //get all customers and resellers which belongs to this user
		{
			$where .= "WHERE parent_id = '0' && (parent_reseller_id = '".$this->session->userdata('customer_id')."' || grand_parent_reseller_id = '".$this->session->userdata('customer_id')."') ";
		}
		else if ($filter_contents == 'my') //only get those which are created by him 
	...
	function export_cdr_data_csv($filter_date_from, $filter_date_to, $filter_phonenum, $filter_customers, $filter_groups, $filter_call_type, $duration_from, $duration_to, $filter_sort, $filter_contents)
	{
		$order_by = "";
        if($filter_sort == 'date_asc')
        {
