...
class Billing extends CI_Controller {
	...
	function index()
	{
		...
		$data['sub_menu']	    =	'default/sub_menu/reseller/billing_sub_menu';
		$data['main_content']	=	'reseller/billing/billing_view';
	...
	function invoices()
	{
		...
		$config['per_page'] = '20';
		$config['base_url'] = base_url().'reseller/billing/invoices/?searchFilter='.$search.'&filter_date_from='.$filter_date_from.'&filter_date_to='.$filter_date_to.'&filter_customers='.$filter_customers.'&filter_billing_type='.$filter_billing_type.'&filter_status='.$filter_status.'&filter_sort='.$filter_sort.'&filter_contents='.$filter_contents.'';
		...
		$data['main_menu']	    =	'default/main_menu/reseller_main_menu';
		$data['sub_menu']	    =	'default/sub_menu/reseller/billing_sub_menu';
		$data['main_content']	=	'reseller/billing/invoices_view';
		$this->load->view('default/template',$data);
	}
    ...
    function my_invoices()
	{
		...
		$this->load->library('pagination');
		$config['per_page'] = '20';
		$config['base_url'] = base_url().'reseller/billing/invoices/?searchFilter='.$search.'&filter_date_from='.$filter_date_from.'&filter_date_to='.$filter_date_to.'&filter_status='.$filter_status.'&filter_sort='.$filter_sort.'';
		$config['page_query_string'] = TRUE;
		...
		$data['count'] = $this->billing_model->get_my_invoices_count($filter_date_from, $filter_date_to, $filter_status);
		$config['total_rows'] = $data['count'];
		...
		$data['invoices']       =   $this->billing_model->get_my_invoices($config['per_page'],$config['uri_segment'],$filter_date_from, $filter_date_to, $filter_status, $filter_sort);

		$data['page_name']		=	'invoices_list';
		$data['selected']		=	'billing';
		$data['sub_selected']   =   'my_invoices';
		$data['page_title']		=	'MY INVOICES';
		$data['main_menu']	    =	'default/main_menu/reseller_main_menu';
		$data['sub_menu']	    =	'default/sub_menu/reseller/billing_sub_menu';
		$data['main_content']	=	'reseller/billing/my_invoices_view';
		$this->load->view('default/template',$data);
	}
